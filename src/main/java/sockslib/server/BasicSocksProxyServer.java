...
public class BasicSocksProxyServer implements SocksProxyServer, Runnable {
  ...
  public void run() {
    ...
    while (!stop) {
      try {
        Socket socket = serverSocket.accept();//从连接队列中取出一个连接,如果没有则等待,此处会新开启一个线程
        logger.info("request is comming");
        socket = processSocketBeforeUse(socket);//返回MonitorSocketWrapper，该类继承socket,对socket进行了封装
        socket.setSoTimeout(timeout);//设置read方法的超时时间，该方法阻塞
        Session session = sessionManager.newSession(socket);//session里有socket对象，socket的输入流输出流之类
        ...
        socksHandler.setSession(session);//通过handler来处理session
        initializeSocksHandler(socksHandler);//handler的初始化设置，给handler设置proxyserver,方法选择器，缓冲大小等

        executorService.execute(socksHandler);//通过定长线程池来执行任务
  ...
  public SocksHandler createSocksHandler() {
    try {
      return socksHandlerClass.newInstance();//newInstance 方法获取该类的一个对象，只能调用无参数的构造方法
  ...
  public void initializeSocksHandler(SocksHandler socksHandler) {
    socksHandler.setMethodSelector(methodSelector);//设置方法选择器,methodSelector 在builder 构建proxy server 时设置
