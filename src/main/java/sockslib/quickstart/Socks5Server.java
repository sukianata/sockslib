...
/**
  ...
  */
  ...
  public static void main(@Nullable String[] args) throws IOException {
    ...
    //String[] params=new String[]{"--port","17211","--it","60000"};
  ...
  public void start(@Nullable String[] args) throws IOException {
    ...
    long inactivetime=0L;
    if (arguments.hasArgsIn("-h", "--help")) {
      showHelp();
      return;
    }
      /**
       * add by huangfan ,means inactive time ,default value:3 minutes
       */
    if (arguments.hasArgsIn("--it")){
        inactivetime=arguments.getLongValue("--it",1000*60*3);
    }
    builder = SocksServerBuilder.newSocks5ServerBuilder();//此处会指定Socks5Handler
    try {
    ...
    server.start();
    SocksServerBuilder finalBuilder = builder;
    long finalInactivetime = inactivetime;
    Thread serviceDaemonThread = new Thread(new Runnable() {
      ...
      public void run() {
        while (true) {
          try {
              /**
               * 规定时间内，链接无活动时断开 add by huangfan 2018/11/22
               */
              Iterator<Map.Entry<Long,Session>> it=server.getSessionManager().getManagedSessions().entrySet().iterator();
              while (it.hasNext()){
                  Map.Entry<Long,Session> entry=it.next();
                  System.out.println(new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new Date())+">>checksession");
                  Object object=entry.getValue().getAttribute(ACTIVETIME);
                  Session session=entry.getValue();
                  if(object!=null&&StringUtils.isNumeric(object.toString())){
                      long lastactivetime=Long.parseLong(object.toString());
                      long now=System.currentTimeMillis();
                      if (now-lastactivetime> finalInactivetime){
                          logger.info(Thread.currentThread().getName()+">>close and remove session:"+session.getId());
                          session.close();
                          server.getSessionManager().sessionOnClose(session);
                          it.remove();
                      }
                  }
              }
            if (!server.isAlive()) {
              logger.info("SOCKS5 sever shutdown , will restart automatically");
