...
public class MonitorDatagramSocketWrapper extends DatagramSocket {
  ...
  public MonitorDatagramSocketWrapper(DatagramSocket datagramSocket, DatagramSocketMonitor...
    ...
    /**
     *
     * & 此处有bug by huangfan 2018/10/11
     *DatagramSocket datagramSocket =  new MonitorDatagramSocketWrapper(clientSocket, networkMonitor);
     *时 ，会先调用父类构造方法，而父类构造方法中调用了子类中重写的方法，子类重写的方法中用到了this.originalDatagramSocket对象、
     *而此时该对象还未初始化，故会报空指针错误
     * 解决办法：把重写的方法中通过originalDatagramSocket对象调用父类方法改为直接调用，把继承后重写但未作改变的方法注释掉，
     */
    this.originalDatagramSocket = datagramSocket;
