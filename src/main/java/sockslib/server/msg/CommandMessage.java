...
public class CommandMessage implements ReadableMessage, WritableMessage {
  ...
  /**socks5 第一次握手成功后 client 发送给 proxy 的信息
  |---------------------------------------------------------------------------|
  ...
  public void read(InputStream inputStream) throws SocksException, IOException {
    ...
    switch (cmd) {
      case CMD_CONNECT:
        command = SocksCommand.CONNECT;//表示要tcp连接 add by huangfan
        break;
      case CMD_BIND:
        command = SocksCommand.BIND;
        break;
      case CMD_UDP_ASSOCIATE:
        command = SocksCommand.UDP_ASSOCIATE;//表示和目的主机要udp连接 add by huangfan
        break;
